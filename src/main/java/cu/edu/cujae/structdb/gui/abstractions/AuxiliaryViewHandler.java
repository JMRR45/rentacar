...
public class AuxiliaryViewHandler extends AbstractViewHandler{
    ...
    public void buttonDelete(DefaultTableModel dtm, int selection) throws ForeignKeyException, ConnectionFailedException {
        switch (type) {
            ...
            case category -> ServicesLocator.categoryServices().remove(list.get(selection).getName());
            case country -> ServicesLocator.countryServices().remove(list.get(selection).getName());
            case paymethod -> ServicesLocator.payMethodServices().remove(list.get(selection).getName());
        }
        refreshDTM(dtm);
    }
